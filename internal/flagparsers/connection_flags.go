...
func AddConnectionFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().String(OutputConnFlag, DefaultOutputConn, "Connection string to use to connect to the output database, i.e. 'user=a password=a host=localhost port=5432 dbname=test sslmode=disable'")
	cmd.PersistentFlags().String(OutputSchemaFlag, DefaultOutputSchema, "The schema of the output database that the data will be inserted into")
}
