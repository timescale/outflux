...
func TestNewForeignKey(t *testing.T) {
	...
	dataSetWithColumns := DataSetInfo{DataSetName: "ds", Columns: []*ColumnInfo{
		&ColumnInfo{Name: goodColumnName, DataType: IDRFInteger32},
	}}
...
func TestNewDataSet(t *testing.T) {
	column, _ := NewColumn("Col 1", IDRFBoolean)
	...
	if dataSet, error := NewDataSet("Data Set", columns, "Col 2"); error == nil || dataSet != nil {
		t.Error("Data Set should have been created, time column not in column set")
	}

	if dataSet, error := NewDataSet("Data Set", columns, "Col 1"); error != nil || dataSet == nil {
...
func TestColumnNamed(t *testing.T) {
	...
	expectedColumnType := IDRFBoolean
